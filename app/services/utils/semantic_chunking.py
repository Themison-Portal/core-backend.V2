...
Optimized for performance while preserving accuracy.
"""
...
# Setup logging (switch to DEBUG for detailed info)
logger = logging.getLogger(__name__)
logger.setLevel(logging.INFO)

# Compile regex patterns globally for reuse
NUMBERED_LIST_PATTERN = re.compile(
    r'^\s*((\d+[\.\)])|([a-zA-Z][\.\)])|(i{1,3}|iv|v|vi{1,3})[\.\)])\s+', re.MULTILINE
)
...
def chunk_text_semantic(
    ...
    logger.debug(f"🔧 SEMANTIC CHUNKING CONFIG: chunk_size={chunk_size}, chunk_overlap={chunk_overlap}")

    # Step 1: Identify sections (headers like "6.1 Inclusion Criteria")
    sections = _split_into_sections(content)
    logger.debug(f"📊 Found {len(sections)} sections")

    # Step 2: Precompute TOC detection only for first N sections (usually 5)
    for i, section in enumerate(sections[:5]):
        section['is_toc'] = _is_toc_section(section['header'], section['content'])
    ...
    logger.debug(f"📊 SEMANTIC CHUNKING RESULTS: {len(chunks)} chunks created from {len(content)} characters")
    if chunks:
        avg_size = sum(len(c.page_content) for c in chunks) / len(chunks)
        toc_count = sum(1 for c in chunks if c.metadata.get('is_toc', False))
        logger.debug(f"   - Average chunk size: {int(avg_size)} chars")
        logger.debug(f"   - Min chunk size: {min(len(c.page_content) for c in chunks)} chars")
        logger.debug(f"   - Max chunk size: {max(len(c.page_content) for c in chunks)} chars")
        logger.debug(f"   - TOC chunks: {toc_count} ({int(toc_count/len(chunks)*100)}%)")
...
def _split_into_sections(content: str) -> List[Dict[str, str]]:
    header_patterns = [
        ...
        (r'^(Table\s+\d+)\s*[-–—:]?\s*(.+)?', 'table'),
        (r'^(Figure\s+\d+)\s*[-–—:]?\s*(.+)?', 'figure'),
    ...
    for line in lines:
        ...
        if TOC_LINE_PATTERN.search(line_stripped):
        ...
        for pattern, section_type in header_patterns:
            ...
            if match:
                ...
                current_content = []
                current_section_start = char_position
                parent_header = None
                section_level = 0
                ...
                if section_type == 'numbered':
                    ...
                    for i in range(len(parts)):
                        ...
                        parent_headers[level_key] = '.'.join(parts[:i+1]) + ' ' + (
                            section_title if i == len(parts)-1 else ''
                        )
                    if section_level > 1:
...
def _is_toc_section(header: str, content: str) -> bool:
    ...
    lines = content.split('\n')
    toc_count = sum(1 for l in lines if l.strip() and l.count('.') >= 5 and re.search(r'\d+\s*$', l.strip()))
    non_empty = [l for l in lines if l.strip()]
    return non_empty and toc_count / len(non_empty) > 0.3


# --- Chunking a Section ---
def _chunk_section(section, chunk_size, chunk_overlap, metadata):
    header = section['header']
    content = section['content']
    parent_header = section.get('parent_header')
    section_start_char = section.get('start_char', 0)
    full_header = f"{parent_header}\n{header}" if parent_header else header
...
def _group_list_items_into_chunks(header, items, chunk_size, chunk_overlap, metadata, section_start_char=0):
    chunks = []
    current_chunk_items = []
    current_size = len(header) + 2
    header_offset = current_size
    current_position = 0
    ...
    for item in items:
        ...
        if current_chunk_items and (current_size + item_size > chunk_size):
            ...
            chunks.append(Document(page_content=chunk_text,
                                   metadata={**metadata, 'chunk_type': 'numbered_list', 'start_index': absolute_start_index}))
            ...
            current_size = header_offset + sum(len(i) + 1 for i in current_chunk_items)
        else:
...
def _get_overlap_items(items: List[str], overlap_size: int) -> List[str]:
    ...
    for item in reversed(items):
        ...
        if total_size + item_size <= overlap_size:
            overlap_items_rev.append(item)
            total_size += item_size
        else:
            break
    return list(reversed(overlap_items_rev))


# --- Regular Text Chunking ---
def _chunk_regular_text(header, content, chunk_size, chunk_overlap, metadata, section_start_char=0):
    from langchain_text_splitters import RecursiveCharacterTextSplitter
    splitter = RecursiveCharacterTextSplitter(
        chunk_size=chunk_size - len(header) - 2,
    ...
    for chunk in content_chunks:
        ...
        result_chunks.append(Document(
            ...
            metadata={**metadata, 'chunk_type': 'regular_text', 'start_index': absolute_start_index}
        ))
